...
To **start it automatically at login**, run the same snippet with the
shortcut path swapped for
`"$env:APPDATA\Microsoft\Windows\Start Menu\Programs\Startup\WhisperFlow.lnk"`.
(Delete the `.lnk` file to undo.)

The icon is generated by `python scripts/make_icon.py` if you want to tweak it.
